...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: true, source: 'sportmonks', live: live.map(enrich), upcoming: upcoming.map(enrich) })
  } catch {
    // Fallback to DB predictions
    try {
      const dbMatches = await dbFallbackMatches()
      const upcoming = dbMatches.slice(0, 8)
      return NextResponse.json({ success: true, source: 'db', live: [], upcoming })
    } catch (e: any) {
