...
export async function GET() {
  try {
    ...
    // Fall back to latest 50 all-time if nothing in last week
    if (records.length === 0) {
    ...
    // Dedupe by matchKey, keep latest per match
    ...
    // Score by confidence + probability gap
    ...
    return NextResponse.json({ success: true, predictions: top })
  } catch (e: any) {
